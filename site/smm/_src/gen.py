...
os.makedirs(OUT_HTML, exist_ok=True); os.makedirs(OUT_PNG, exist_ok=True)
...
def nb(s):
    ...
    # предлоги и союзы
    for _ in range(2):
        s = re.sub(r'(^|[\s(«])' + SHORT + r'\s', lambda m: m.group(1) + m.group(2) + NB, s, flags=re.I)
    return s
...
def P(name, kick, inner, i, n, net=False, cls=''):
    slides.append((name, 'post', page('post', inner, '%d / %d' % (i, n), kick, net, cls)))

def S(name, kick, inner, net=False, cls='', logo=True):
    slides.append((name, 'story', page('story', inner, None, kick, net, cls, logo=logo)))

# ПОСТ 1 – про нас, команду и миссию (главная synapt.tech)
N = 7
P('post1-01', 'студия', thesis('Студия AI и IT-разработки для бизнеса', 'Соединяем AI, процессы и людей в устойчивую рабочую систему', 'xl'), 1, N, net=True)
P('post1-02', 'почему мы', thesis('Объединяем компанию с AI и ускоряем прогресс', 'Синапс – место, где сигнал переходит от одного нейрона к следующему, и из таких точек собрана любая нейросеть. Для того же работает наша команда: приходим в компанию и соединяем её процессы и людей с новыми технологиями', 'm'), 2, N)
P('post1-03', 'миссия', thesis('Мы хотим, чтобы после нас задачи решались быстрее, показатели росли, а люди занимались тем, что двигает их и бизнес вперёд', 'Каждая такая связка помогает эволюционировать не только компании, но и человечеству', 'm'), 3, N)
...
P('post1-07', 'следующий шаг', cta('30 минут в Zoom с партнёрами студии', 'Вы рассказываете про бизнес, мы задаём вопросы и совместно определяем цели. После присылаем запись разговора и разбор: что увидели и с чего рекомендуем начать'), 7, N, net=True)
...
S('story-02-steps-5', 'следующий шаг', cta('Можно начать с небольшого решения?', 'Да, чекап за две недели или компактное решение под один процесс. Дальше добавляем модули по тому, что показала работа первого'), net=True)
...
for name, fmt, h in slides:
    ...
    sc = scale(fmt)
    manifest.append(dict(html=p, png=os.path.join(OUT_PNG, name + '.png'), w=sc['w'], h=sc['h']))
