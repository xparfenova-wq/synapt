...
OUT_PNG = os.path.expanduser('~/business-shared/_drafts/synapt-repo/site/strategy/assets')
...
.frame.fr-lime .it .t,.frame.fr-milk .it .t,.frame.fr-lime .nm,.frame.fr-milk .nm,.frame.fr-lime .n,.frame.fr-milk .n{color:#12150A}
.frame.fr-lime .it .s,.frame.fr-lime .rl,.frame.fr-lime .cap{color:#3E4A1E}
.frame.fr-milk .it .s,.frame.fr-milk .rl,.frame.fr-milk .cap{color:#5A6560}
.frame.fr-lime .card,.frame.fr-milk .card,.frame.fr-lime .it,.frame.fr-milk .it{background:rgba(18,21,10,.05);border-color:rgba(18,21,10,.18)}
.frame.fr-lime .chip,.frame.fr-milk .chip{color:#12150A;border-color:rgba(18,21,10,.3)}
.frame .chip{border:0!important;padding:0!important;border-radius:0!important;background:none!important;letter-spacing:.06em}
.frame.fr-milk .btn{background:#12150A;color:#F4F6F5}
.frame.fr-lime .num,.frame.fr-milk .num{color:#12150A}
.frame.fr-lime .num .tail,.frame.fr-milk .num .tail{color:#3E4A1E}
.frame.tight .it .t{font-size:42px;line-height:1.25}
.frame.tight .it .s{font-size:36px;line-height:1.3}
.frame.tight .it{padding:22px 26px}
.frame.tight .list{gap:14px}
.frame.tight h1.m{font-size:64px}
...
def cards(title, items, on_last=False, text=None, tsize='m', all_on=False, row=False, kick_chip=None):
    b = '<div class="body">'
    if kick_chip: b += '<span class="chip">%s</span>' % nb(kick_chip)
    if title: b += '<h1 class="%s">%s</h1>' % (tsize, nb(title))
    if text: b += '<p class="tx">%s</p>' % nb(text)
...
HL_VARIANTS = {1: ('#D0E865', '#12150A'), 2: ('#151A18', '#FFFFFF'), 3: ('#151A18', '#D0E865'), 4: ('#151A18', '#FFFFFF'), 5: ('#F4F6F5', '#12150A'), 6: ('#F4F6F5', '#12150A')}
...
def P(name, kick, inner, i, n, net=False, cls='', logo=False):
    # Посты без знака и подписи: человек уже в нашем профиле
    slides.append((name, 'post', page('post', inner, '%d / %d' % (i, n), kick, net, cls, logo=False, site=False)))
...
N = 7
P('post1-01', 'студия', thesis('Встраиваем AI в работу компании', 'Разбираем бизнес-процессы и собираем устойчивую систему с AI, которая ускоряет работу и остаётся у вас', 'xl'), 1, N, net=True, logo=True)
P('post1-02', 'почему мы', thesis('Объединяем компанию с AI и ускоряем прогресс', 'Синапс – место, где сигнал переходит от одного нейрона к следующему. Для того же работает наша команда: соединяем процессы и людей компании с новыми технологиями', 'm'), 2, N, logo=True)
...
P('post1-04', 'почему сейчас', cards('AI внедряют почти все, вопрос в том, как', [
    ('одни', 'покупают подписки и раздают инструменты. Работа остаётся прежней'),
    ('другие', 'перестраивают сам процесс: кто принимает заявку, откуда берутся знания'),
], on_last=True, tsize='m'), 4, N, logo=True)
TEAM_PH = '/Users/xeniaparfenova/business-shared/_drafts/synapt-team-photos'
...
P('post1-07', 'следующий шаг', cta('30 минут в Zoom с партнёрами студии', 'Вы рассказываете про бизнес, мы задаём вопросы. После присылаем запись и разбор: с чего рекомендуем начать'), 7, N, net=True, logo=True)

# ПОСТ 3 – услуги: три шага и модули системы (главная synapt.tech, 01 Услуги; цена десанта с synapt.tech/desant)
P('post3-01', 'услуги', thesis('Сначала формируем карту процессов бизнеса, затем строим систему под неё', 'Изучаем, где компания теряет деньги, и собираем под это AI-систему', 'l'), 1, N, net=True, cls='fr-lime')
P('post3-02', 'шаг 1 из 3', metrics('AI-чекап бизнеса', [
    ('19 900 ₽', 'две недели, сумма фиксированная'),
    ('14 дней', 'от Zoom до карты процессов и первого агента'),
], text='Карта процессов с узкими местами, приоритеты по эффекту и план внедрения с вилкой бюджета', size='sm', tsize='l'), 2, N)
P('post3-03', 'шаг 2 из 3', metrics('AI-десант и обучение команды', [
    ('30 дней', 'один специалист, полный рабочий месяц'),
    ('от 80 тыс. ₽', 'месяц работы специалиста внутри вашей команды'),
], text='Специалист студии месяц встраивает нейросети в задачи каждого отдела', size='sm', tsize='l'), 3, N)
P('post3-04', 'шаг 3 из 3', metrics('AI-система под каждый бизнес-процесс', [
    ('3-5 недель', 'до готового продукта'),
    ('60 дней', 'на связи после завершения'),
], text='Какие блоки нужны именно вам, показывают AI-чекап и AI-десант', size='sm', tsize='l'), 4, N)
P('post3-05', 'модули системы', lst('Система разрабатывается из блоков', [
    ('продажи', 'Работа с клиентом'),
    ('аналитика', 'AI-дашборд для бизнеса'),
...
P('post3-06', 'не нашли свою задачу', thesis('Список блоков – частые запросы, не весь возможный набор', 'Если процесс повторяется и по нему есть данные, соберём под него решение. Задача больше блока – SaaS, приложение, внутренняя система – возьмём целиком', 'l'), 6, N)
P('post3-07', 'следующий шаг', cta('30 минут в Zoom с партнёрами студии', 'Вы рассказываете про бизнес, мы задаём вопросы. После присылаем запись и разбор: с чего рекомендуем начать'), 7, N, net=True)

# ПОСТ 2 – как работаем: шесть принципов и этапы (главная synapt.tech, 05 Подход и 03 Как работаем)
PRINC = [
    ('Сначала проводим анализ, потом определяем стоимость и критерии', 'Первый Zoom – погружение в запрос. Затем анализ, форматы сотрудничества и критерии приёмки в договоре'),
    ('Работаем быстро и итеративно', 'Первый результат через 3-5 недель. Каждую неделю показываем прогресс'),
    ('Система остаётся у вас целиком', 'Код, доступы, промпты, база знаний и документация – ваши. Продолжить можно с нами или с другой командой'),
    ('Данные не покидают ваш контур', 'Фиксируем, какие поля уходят в модель. Обезличивание, ролевой доступ и логирование настраиваем мы, юрист получает описание контура'),
    ('60 дней на связи после сдачи', 'Обкатываем систему на реальной работе и донастраиваем под новые сценарии'),
    ('Берёмся только за то, что умеем', 'AI-агенты, интеграции, веб и мобильная разработка. Если задача решается процессом и AI не нужен – скажем'),
]
P('post2-01', 'подход', thesis('Шесть принципов, по которым работаем над каждым проектом', 'Смету и критерии фиксируем в договоре до старта, результат показываем каждую неделю', 'l'), 1, N, net=True, cls='fr-milk')
P('post2-02', 'принципы 1 и 2 из 6', plist(PRINC, 0), 2, N)
P('post2-03', 'принципы 3 и 4 из 6', plist(PRINC, 2), 3, N)
P('post2-04', 'принципы 5 и 6 из 6', plist(PRINC, 4), 4, N)
P('post2-05', 'как работаем', lst('Декомпозируем проект на этапы и каждый завершаем конкретным результатом', [
...
P('post2-06', 'о сроках', thesis('Длительность этапов у проектов разная: разбор может занять неделю или три', 'Сроки и стоимость фиксируем в спецификации после встречи', 'm'), 6, N)
P('post2-07', 'следующий шаг', cta('30 минут в Zoom с партнёрами студии', 'Вы рассказываете про бизнес, мы задаём вопросы. После присылаем запись и разбор: с чего рекомендуем начать'), 7, N, net=True)

# СТОРИС – хайлайт 1 «о студии» (главная synapt.tech: hero, Что предлагаем, Почему мы, команда)
S('story-01-studio-1', 'студия', thesis('Встраиваем AI в работу компании', 'Разбираем бизнес-процессы и собираем устойчивую систему с AI, которая ускоряет работу и остаётся у вас', 'xl', center=True), net=True)
S('story-01-studio-2', 'что предлагаем', lst('Четыре запроса, с которыми к нам приходят', [
    ('AI-чекап: покажем, где внедрение окупится', 'хотите применить AI, но не знаете, с чего начать'),
    ('AI-десант: настраиваем процессы внутри команды', 'хотите, чтобы команда начала регулярно работать с AI'),
    ('AI-система: разработка с первого нужного модуля', 'точно знаете, какой процесс надо оптимизировать'),
    ('Напишите нам: разберём и предложим, с чего начать', 'есть запрос на ускорение, но не похоже на первые три'),
]), cls='graphite tight')
S('story-01-studio-3', 'миссия', thesis('Мы хотим, чтобы после нас задачи решались быстрее, показатели росли, а люди занимались тем, что двигает их и бизнес вперёд', 'Каждая такая связка помогает эволюционировать не только компании, но и человечеству', 'm'), net=True)
S('story-01-studio-4', 'команда', team('Кто ведёт проект', [
    ('Виктор Калугин', 'технический партнёр, архитектура системы', TEAM_PH + '/витя.jpg'),
    ('Антон Орешкин', 'стратегический партнёр, продукт и развитие', TEAM_PH + '/антон.jpg'),
    ('Никита', 'project-manager студии', TEAM_PH + '/никита.jpg'),
]), cls='fr-milk')
S('story-01-studio-5', 'следующий шаг', cta('30 минут в Zoom с партнёрами студии', 'Вы рассказываете про бизнес, мы задаём вопросы. После присылаем запись и разбор: с чего рекомендуем начать'), net=True)
...
S('story-02-steps-4', 'шаг 3 из 3', metrics('AI-система под каждый бизнес-процесс', [
    ('3-5 недель', 'до готового продукта'),
    ('60 дней', 'на связи после завершения'),
...
# СТОРИС – хайлайт «кейсы» (все карточки раздела Кейсы на synapt.tech, дословно)
S('story-03-cases-1', 'кейс, AI-десант', metrics('Поиск фабрики занимает 2 минуты, раньше уходило 10', [
    ...
    ('1 месяц', 'десанта внутри команды на конкретных задачах'),
...
], on_last=True, tsize='m', kick_chip='Оптовая компания, имя под NDA'))
...
S('story-03-cases-5', 'кейс, AI-консультант', metrics('Консультант разбирает запрос по методике компании прямо в Telegram', [
    ('300+', 'часов работы экспертов экономим каждый месяц'),
    ('800+', 'страниц методики внутри базы знаний'),
], kick_chip='Консалтинговая компания, имя под NDA', size='mid', tsize='l'))
S('story-03-cases-6', 'кейс, разработка', metrics('30 лет расчётов переехали в браузер без расхождений', [
    ('100%', 'совпадение расчётов со старой системой'),
    ('30+ лет', 'накопленной логики сохранено'),
], kick_chip='Инженерное бюро, имя под NDA', size='mid', tsize='l'))
S('story-03-cases-7', 'кейс, b2c-сервис', metrics('Юридический консультант отвечает на вопросы в чате', [
    ('24/7', 'ответ в любое время'),
    ('2-5 сек', 'ответ в обычном режиме, до минуты в экспертном'),
], kick_chip='Коммерческий продукт, участие в разработке', size='mid', tsize='l'))
S('story-03-cases-8', 'кейс, MCP-агент', metrics('MCP-агент для подготовки корпоративной отчётности', [
    ('2 мин', 'на отчёт вместо 5 часов'),
    ('3', 'источника: 1С, Sheets, PostgreSQL'),
], kick_chip='Операционный отдел, имя под NDA', size='mid', tsize='l'))
S('story-03-cases-9', 'кейс, экономическая безопасность', metrics('Агент сам проверяет контрагентов по базе знаний', [
    ('1,8 мин', 'на проверку вместо 17'),
    ('1 267', 'человеко-часов высвобождено'),
], kick_chip='Отдел экономической безопасности, имя под NDA', size='mid', tsize='l'))
S('story-03-cases-10', 'кейс, голосовой агент', metrics('Голосовой агент прозванивает базу и назначает встречи менеджеру', [
    ('до 200', 'звонков в день'),
    ('встреча', 'итог звонка с менеджером'),
], kick_chip='Отдел продаж, имя под NDA', size='mid', tsize='l'))
...
S('story-04-system-3', 'подход', lst('Шесть принципов, по которым работаем над каждым проектом', [
    ('Сначала проводим анализ, потом определяем стоимость и критерии',),
...
S('story-05-faq-5', 'следующий шаг', cta('30 минут в Zoom с партнёрами студии', 'Вы рассказываете про бизнес, мы задаём вопросы. После присылаем запись и разбор: с чего рекомендуем начать'), net=True, logo=True)

# СТОРИС – хайлайт «подход» (главная synapt.tech, раздел Подход, дословно)
S('story-06-approach-1', 'подход, 1 из 6', thesis('Сначала проводим анализ, потом определяем стоимость и критерии', 'Первый Zoom – погружение в запрос и контекст. Затем внутренний анализ, форматы сотрудничества на выбор и чёткие критерии приёмки в договоре', 'l'), net=True)
S('story-06-approach-2', 'подход, 2 из 6', thesis('Работаем быстро и итеративно', 'Первый результат отдаём команде через 3-5 недель. Каждую неделю показываем прогресс и дорабатываем по обратной связи', 'l'), cls='fr-lime')
S('story-06-approach-3', 'подход, 3 из 6', thesis('Система остаётся у вас целиком', 'Передаём вам всю систему: код, доступы, промпты, базу знаний и документацию. Продолжить работу можно с нами или с другой командой', 'l'))
S('story-06-approach-4', 'подход, 4 из 6', thesis('Данные не покидают ваш контур', 'Фиксируем, какие поля уходят в модель. Обезличивание, ролевой доступ и логирование настраиваем мы, ваш юрист получает описание контура', 'l'), cls='fr-milk')
S('story-06-approach-5', 'подход, 5 из 6', thesis('60 дней на связи после сдачи', 'В первые недели обкатываем систему на работе, и если что-то идёт не так, правим. Заодно быстро донастраиваем под нагрузку и новые сценарии, которые появляются по ходу', 'l'))
S('story-06-approach-6', 'подход, 6 из 6', thesis('Берёмся только за то, что умеем', 'AI-агенты, интеграции, веб и мобильная разработка для бизнеса. Если задача решается на уровне процесса и AI не нужен – тоже скажем', 'l'), cls='graphite')
S('story-06-approach-7', 'следующий шаг', cta('30 минут в Zoom с партнёрами студии', 'Вы рассказываете про бизнес, мы задаём вопросы. После присылаем запись и разбор: с чего рекомендуем начать'), net=True)

# ОБЛОЖКИ хайлайтов
for i, w in enumerate(['о студии', 'три шага', 'кейсы', 'система', 'вопросы', 'подход'], 1):
